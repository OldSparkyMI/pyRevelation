...
from src.bundle import PBKDFv2, AfSplitter
...
class LuksFile:
    ...
    def create(self, file, cipherName, cipherMode, hashSpec, masterSize, stripes):
        ...
        self.mkDigest = pbkdf.makeKey(self.masterKey, self.mkDigestSalt, self.mkDigestIterations,
                                      hashlib.new(self.hashSpec).digest_size, self.hashSpec)
    ...
    def open_key(self, keyIndex, password):
        ...
        checkDigest = pbkdf.makeKey(masterKey, self.mkDigestSalt, self.mkDigestIterations,
                                    hashlib.new(self.hashSpec).digest_size, self.hashSpec)
    ...
    def _uuidgen(self, rand):
        ...
        self.uuid = "%08x-%04x-%04x-%02x%02x-%02x%02x%02x%02x%02x%02x" % (
        low, mid, hi, seq >> 8, seq & 0xFF, b5, b4, b3, b2, b1, b0)
    ...
    def _save_header(self):
        ...
        str = struct.pack(self.LUKS_FORMAT, self.magic, self.version, self.cipherName, self.cipherMode, self.hashSpec, \
                          self.payloadOffset, self.keyBytes, self.mkDigest, self.mkDigestSalt, self.mkDigestIterations,
                          self.uuid)
        self.file.seek(0)
        self.file.write(str)
