...
STOCK_ICONS = (
    ...
    (STOCK_REVELATION_LOCKED, "revelation-locked",
     (ICON_SIZE_APPLET, ICON_SIZE_LOGO, Gtk.IconSize.DIALOG, Gtk.IconSize.MENU)),
    (STOCK_ENTRY_CREDITCARD, "contact-new",
     (ICON_SIZE_DATAVIEW, ICON_SIZE_DROPDOWN, ICON_SIZE_ENTRY, ICON_SIZE_TREEVIEW)),
    (STOCK_ENTRY_CRYPTOKEY, "dialog-password",
     (ICON_SIZE_DATAVIEW, ICON_SIZE_DROPDOWN, ICON_SIZE_ENTRY, ICON_SIZE_TREEVIEW)),
    (STOCK_ENTRY_DATABASE, "package_system",
     (ICON_SIZE_DATAVIEW, ICON_SIZE_DROPDOWN, ICON_SIZE_ENTRY, ICON_SIZE_TREEVIEW)),
    ...
    (STOCK_ENTRY_FTP, "system-file-manager",
     (ICON_SIZE_DATAVIEW, ICON_SIZE_DROPDOWN, ICON_SIZE_ENTRY, ICON_SIZE_TREEVIEW)),
    (
        STOCK_ENTRY_GENERIC, "document-new",
        (ICON_SIZE_DATAVIEW, ICON_SIZE_DROPDOWN, ICON_SIZE_ENTRY, ICON_SIZE_TREEVIEW)),
    (STOCK_ENTRY_PHONE, "phone", (ICON_SIZE_DATAVIEW, ICON_SIZE_DROPDOWN, ICON_SIZE_ENTRY, ICON_SIZE_TREEVIEW)),
    (STOCK_ENTRY_SHELL, "utilities-terminal",
     (ICON_SIZE_DATAVIEW, ICON_SIZE_DROPDOWN, ICON_SIZE_ENTRY, ICON_SIZE_TREEVIEW)),
    (STOCK_ENTRY_REMOTEDESKTOP, "preferences-desktop-remote-desktop",
     (ICON_SIZE_DATAVIEW, ICON_SIZE_DROPDOWN, ICON_SIZE_ENTRY, ICON_SIZE_TREEVIEW)),
    ...
    (STOCK_ENTRY_FOLDER_OPEN, "folder-open",
     (ICON_SIZE_DATAVIEW, ICON_SIZE_DROPDOWN, ICON_SIZE_ENTRY, ICON_SIZE_TREEVIEW)),
)
...
class ComboBoxEntry(Gtk.ComboBox):
    ...
    def __init__(self, list=[]):
        ...
        self.model = Gtk.ListStore(GObjectTYPE_STRING)
...
class IconEntry(Alignment):
    ...
    def __cb_expose(self, widget, data):
        ...
        style.paint_flat_box(self.window, self.entry.state, Gtk.SHADOW_NONE, None, self.entry, "entry_bg", x, y, width,
                             height)
...
class PasswordEntryGenerate(HBox):
    ...
    def generate(self):
        ...
        password = util.generate_password(self.config.get("passwordgen/length"),
                                          self.config.get("passwordgen/punctuation"))
...
class DropDown(Gtk.ComboBox):
    ...
    def __init__(self, icons=False):
        ...
        self.model = Gtk.ListStore(GObjectTYPE_STRING, GObjectTYPE_STRING, GObjectTYPE_PYOBJECT)
        self.set_model(self.model)
...
class TreeView(Gtk.TreeView):
    ...
    def __cb_buttonpress(self, widget, data):
        ...
        elif self.__cbid_drag_motion == None and data.button in (
                1, 2) and data.type == Gtk.gdk.BUTTON_PRESS and path != None and self.selection.iter_is_selected(
            self.model.get_iter(path[0])) == True and len(self.get_selected()) > 1:
    ...
    def __cb_drag_motion(self, widget, data, userdata=None):
        ...
        if self.drag_check_threshold(int(userdata.x), int(userdata.y), int(data.x), int(data.y)) == True:
            self.__drag_check_end()
            self.drag_begin((("revelation/treerow", Gtk.TARGET_SAME_APP | Gtk.TARGET_SAME_WIDGET, 0),),
                            Gtk.gdk.ACTION_MOVE, userdata.button, userdata)
...
class ItemFactory(Gtk.IconFactory):
    ...
    def create_stock_item(self, id, name, icon=None):
        ...
        stockItem.stock_id = id
        stockItem.label = name
        ...
        if icon is None:
            ...
            self.create_stock_icon(id, icon, (
                Gtk.IconSize.SMALL_TOOLBAR, Gtk.IconSize.LARGE_TOOLBAR, Gtk.IconSize.MENU, Gtk.IconSize.BUTTON,
                Gtk.IconSize.DIALOG, ICON_SIZE_LABEL, ICON_SIZE_HEADLINE))

    def get_iconsource(self, id, size, wildcard=False):
        """Loads an icon as an iconsource"""

        successfully, width, height = Gtk.IconSize.lookup(size)
